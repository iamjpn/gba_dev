...
void ball_init(Ball *ball) {
  ...
  obj_set_attr(
      ball->sprite, OBJ_Y(from_fix(ball->y)) | ATTR0_COLOR_16 | ATTR0_SQUARE,
      OBJ_X(from_fix(ball->x)) | ATTR1_SIZE_8, ATTR2_PALBANK(0) | 0x08);
}
...
bool collided2(Ball *ball, Paddle *paddle, int *xSpeed, int *ySpeed) {
  ...
  if (ballCoord.x + (BALL_WIDTH * addBallWidth) ==
      paddleCoord.x + (PADDLE_WIDTH * addWidth)) {
